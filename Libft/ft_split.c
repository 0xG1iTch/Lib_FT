...
size_t	count_splits(const char *s, char c)
{
	size_t	counter;
	int		flag;
...
void	free_space(char **splits, size_t i)
{
	while (i > 0)
	{
		free(splits[i - 1]);
...
char	*word_set(const char *s, char c, size_t *index)
{
	char	*word;
	size_t	start;
	size_t	length;
